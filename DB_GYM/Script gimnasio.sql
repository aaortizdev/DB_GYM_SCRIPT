...
-- TRIGGER tgr_insert_socio que válida que una persona sea de tipo socio en la tabla Socio
...
-- TRIGGER tgr_insert_empleado que válida que una persona sea de tipo empleado en la tabla Empleado
...
-- TRIGGER tgr_insert_clase que válida que un empleado sea de tipo instructor en la tabla Clase 
drop trigger if exists tgr_insert_clase;
delimiter %%
...
-- TRIGGER tgr_insert_mant que válida que un empleado sea de tipo técnico en la tabla Mantenimiento
drop trigger if exists tgr_insert_mant;
delimiter %%
...
-- Socios que tienen una membresia activa 
select s.socio_id, p.nombre, p.apellido, m.tipo, m.fecha_inicio, m.fecha_fin
from Membresia m
inner join Socio s on m.socio_id = s.socio_id
inner join Persona p on s.persona_id = p.persona_id
where CURDATE() between m.fecha_inicio and m.fecha_fin;

-- Socios con su Objetivo de Entrenamiento
select p.nombre, p.apellido, o.descripcion as objetivo
from ObjetivoEntrenamiento o
inner join Socio s on o.socio_id = s.socio_id
inner join Persona p on s.persona_id = p.persona_id;
...
BEGIN
    ...
    select LAST_INSERT_ID() into @v_pago_id;   
...
## trigger
-- Control de Stock en las Ventas
drop trigger if exists tgr_control_stock;

delimiter %%
create trigger tgr_control_stock before insert on detalleventa
for each row
begin
    ...
    select stock into @producto_stock from producto where producto_id = new.producto_id;
    if @producto_stock < new.cantidad then
        signal sqlstate '45000' set message_text = 'Stock insuficiente para este producto';
	end if;
    
    set @stock_actualizado = @producto_stock - new.cantidad;
    update producto
